...
/// #ref:functions
public fun mint_deposit_cap(trading_account: &mut TradingAccount, ctx: &mut TxContext): DepositCap {
    trading_account.validate_owner(ctx);
...
/// #ref:functions
public fun deposit<T>(trading_account: &mut TradingAccount, coin: Coin<T>, ctx: &mut TxContext) {
...
public fun withdraw_with_cap<T>(
    ...
    let coin = trading_account.withdraw_with_proof(&proof, withdraw_amount, false).into_coin(ctx);
    trading_account.emit_balance_event(
        type_name::with_defining_ids<T>(),
        coin.value(),
        false,
...
// === Private Functions ===
fun mint_trade_cap_internal(trading_account: &mut TradingAccount, ctx: &mut TxContext): TradeCap {
...
fun validate_trader(trading_account: &TradingAccount, trade_cap: &TradeCap) {
    assert!(trading_account.allow_listed.contains(object::borrow_id(trade_cap)), EInvalidTrader);
}

fun validate_deposit_cap(trading_account: &TradingAccount, deposit_cap: &DepositCap) {
    assert!(trading_account.allow_listed.contains(object::borrow_id(deposit_cap)), EInvalidTrader);
}

fun validate_withdraw_cap(trading_account: &TradingAccount, withdraw_cap: &WithdrawCap) {
    assert!(trading_account.allow_listed.contains(object::borrow_id(withdraw_cap)), EInvalidTrader);
}
