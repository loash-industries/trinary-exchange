...
///
/// Coin-pool fork of `triex::vault`, which now retains only the fee-event
/// helpers that `triex::multicoin_vault` shares. Identical apart from the `u128`
/// order ids on the refund path. The escrow accounting is shared verbatim with
/// the multicoin vault's own copy — any change to it must land in both.
