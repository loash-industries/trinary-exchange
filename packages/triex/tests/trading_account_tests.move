...
fun test_deposit_multicoin_with_wrong_cap_e() {
    ...
    test.next_tx(BOB);
    {
        let mut trading_account_2 = test.take_shared_by_id<TradingAccount>(trading_account_id_2);
        let deposit_cap = test.take_from_sender<DepositCap>();
...
fun test_multicoin_mixed_with_regular_coins_ok() {
    ...
    let multicoin_balance_after = trading_account.multicoin_balance(collection_id, TEST_ASSET_ID_1);
