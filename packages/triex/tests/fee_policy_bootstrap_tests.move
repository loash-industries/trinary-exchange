...
#[test]
/// Multicoin is the premium venue: every rung costs exactly twice its coin-pool
/// counterpart. Pins the direction of the split, which is easy to invert.
fun multicoin_prices_at_twice_the_coin_pool() {
    ...
    let mut i = 0;
    while (i < GENESIS_TIERS) {
        assert_eq!(mc_t[i], coin_t[i] * 2);
        assert_eq!(mc_m[i], coin_m[i] * 2);
        // And makers always undercut takers on the same rung.
...
fun thresholds_scale_with_quote_decimals() {
    ...
    policy.bootstrap_quote<NINE_DP>(COIN_CLASS, MULTICOIN_CLASS, NINE_DP_UNIT, &cap, test.ctx());
...
fun each_threshold_promotes_on_its_own_boundary() {
    ...
    let (top, top_taker, _) = policy.resolve(COIN_CLASS, thresholds[GENESIS_TIERS - 1] * 1_000, epoch);
