...
module triex::ewma_tests {
    ...
    fun advance_scenario_with_gas_price(
        test: &mut Scenario,
        gas_price: u64,
        timestamp_advance: u64,
    ) {
    ...
    fun test_apply_taker_penalty_disabled_ewma() {
        ...
        let base_taker_fee = constants::maybe_apply_fee(true); // Current unified fee rate
        let mut ewma_state = test_init_ewma_state(test.ctx());
    ...
    fun test_apply_taker_penalty_gas_above_mean_below_threshold() {
        ...
        let base_taker_fee = constants::maybe_apply_fee(true); // Current unified fee rate
        let additional_fee = constants::maybe_apply_fee(true); // Use same rate for additional
        let mut ewma_state = test_init_ewma_state(test.ctx());
    ...
    fun test_apply_taker_penalty_z_score_above_threshold() {
        ...
        let base_taker_fee = constants::maybe_apply_fee(true); // Current unified fee rate
        let additional_fee = base_taker_fee / 2; // Half of base fee for additional
        let mut ewma_state = test_init_ewma_state(test.ctx());
        ewma_state.set_additional_taker_fee(additional_fee);
        ewma_state.set_z_score_threshold(1_000_000_000); // 1 std dev
        ewma_state.enable();
    ...
    fun test_dynamic_additional_taker_fee_changes() {
        ...
        let base_taker_fee = constants::maybe_apply_fee(true); // Current unified fee rate
        let mut ewma_state = test_init_ewma_state(test.ctx());
        let first_additional = base_taker_fee / 4; // Quarter of base fee
        ewma_state.set_additional_taker_fee(first_additional); // Initially
        ...
        assert_eq!(fee_1, base_taker_fee + first_additional);

        // Change additional taker fee
        let second_additional = base_taker_fee * 3 / 4; // Three quarters of base fee
        ewma_state.set_additional_taker_fee(second_additional);

        // Same conditions, different penalty
        let fee_2 = ewma_state.apply_taker_penalty(base_taker_fee, test.ctx());
        assert_eq!(fee_2, base_taker_fee + second_additional);
