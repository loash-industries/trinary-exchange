...
module triex::book_admission_window_tests {
    ...
    fun run_grid(is_bid: bool) {
        ...
        let mut hi = 0;
        while (hi < occupancies.length()) {
            let h = occupancies[hi];
            let mut ti = 0;
    ...
    fun admission_grid_asks() { run_grid(false) }
    ...
    /// side and fail here.
    fun admission_grid_bids() { run_grid(true) }
    ...
    fun repeated_window_cycles() {
        ...
        while (true) {
            ...
            while (cycle < 5) {
                ...
                let mut k = 1;
                while (k < 6) {
                    rest(&mut book, improve(is_bid, best_tree_px, STRIDE / 2 + k * STRIDE), is_bid);
